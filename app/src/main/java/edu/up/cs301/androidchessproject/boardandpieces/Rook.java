...
public class Rook extends ChessPiece {
    ...
    public static boolean isValidRookMove(ChessState state, int rowStart, int colStart, int rowEnd, int colEnd){
        ...
        ChessPiece endPiece = state.getBoard().getSquares()[rowStart][colStart].getPiece();
        ...
        try{
            ...
            if (colDiff == 0 && state.getBoard().areSquaresOnLineEmpty(true, rowStart, rowEnd, colEnd)){
                if (!state.getBoard().getSquares()[rowEnd][colEnd].hasPiece()){
                    return true;
                }
                else if (Math.abs(state.getBoard().getSquares()[rowEnd][colEnd].getPiece().getBlackOrWhite() - color) == 1){
                    return true;
                }
            }
